        ...
        <div class="container">
            <form action="<?php echo $id != null ? 'CRUDLogic/editOuvrage.php?' . $id[0] : 'CRUDLogic/createOuvrage.php' ?>" id="ouvrage-form" class="mt-4" method="post">
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="nom" name="nom" placeholder="Nom de l'ouvrage" value="<?= isset($nom) ? $nom : '' ?>" required>
                </div>
                <div class="mb-3">
                    <label for="type" class="form-label">Type</label>
                    <input type="text" class="form-control" id="type" name="type" placeholder="Type de l'ouvrage" value="<?= isset($type) ? $type : '' ?>" required>
                </div>
                <div class="mb-3">
                    <label for="auteur" class="form-label">Auteur</label>
                    <input type="text" class="form-control" id="auteur" name="auteur" placeholder="Nom & Prénom de l'auteur" value="<?= isset($auteur) ? $auteur : '' ?>" required>
                </div>
                <div class="mb-3">
                    <label for="date" class="form-label">Année de parution</label>
                    <input type="number" class="form-control" id="date" name="date" placeholder="AAAA" value="<?= isset($date) ? $date : '' ?>" required>
                </div>
                <div class="mb-3">
                    <label for="descriptif" class="form-label">Descriptif</label>
                    <textarea class="form-control" id="descriptif" name="descriptif" rows="4" required placeholder="Descriptif de l'ouvrage"><?= isset($descriptif) ? $descriptif : '' ?></textarea>
                </div>
